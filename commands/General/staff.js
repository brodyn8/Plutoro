...
class Staff extends Command {
	...
	async run (message, args, data) {
		...
		const embed = new Discord.MessageEmbed()
			...
			.addField(message.translate("general/staff:ADMINS"), (administrators.size > 0 ? administrators.map((a) => `${a.user.tag}`).join("\n") :message.translate("general/staff:NO_ADMINS")))
			.addField(message.translate("general/staff:MODS"), (moderators.size > 0 ? moderators.map((m) => `${m.user.tag}`).join("\n") : message.translate("general/staff:NO_MODS")))
