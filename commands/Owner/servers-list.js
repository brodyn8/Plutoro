...
class ServersList extends Command {
	...
	async run (message, args, data) {
		...
		this.client.guilds.cache.sort((a,b) => b.memberCount-a.memberCount).map((r) => r)
			.map((r, i) => `**${i + 1}** - ${r.name} | ${r.memberCount} ${message.translate("common:MEMBERS").toLowerCase()} | ${r.id}`)
			.slice(0, 10)
			.join("\n");
		...
		const collector = msg.createReactionCollector((reaction, user) => user.id === message.author.id, { time: 60000 } // 1 minute
    );
		...
		collector.on("collect", async(reaction) => {
			...
			if(reaction._emoji.name === "⬅") {
				...
				this.client.guilds.cache.sort((a,b) => b.memberCount-a.memberCount).map((r) => r)
					.map((r, i) => `**${i + 1}** - ${r.name} | ${r.memberCount} ${message.translate("common:MEMBERS")} | ${r.id}`)
					.slice(i0, i1)
					.join("\n");
